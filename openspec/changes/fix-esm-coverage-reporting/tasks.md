...
- [ ] 1.1 Remove `nyc` from `devDependencies` in `cli-app/package.json` and add `c8` pinned to
      `9.1.0`; verify `yarn` installs without an engine warning under the Node version in
      `cli-app/.nvmrc`
- [ ] 1.2 Point the `coverage` script in `cli-app/package.json` at `c8` wrapping the existing mocha
      invocation, leaving the `test` script untouched; verify `yarn test` output is byte-identical
      to before the change
- [ ] 1.3 Run `grep -rn "nyc\|\.nycrc" --exclude-dir=node_modules .` across the repository; verify
...
- [ ] 2.1 Run coverage with the pinned `c8` under Node 18; verify it reports non-zero coverage,
      confirming instrumentation now reaches ESM modules at all
- [ ] 2.2 Repeat under Node 16, the matrix's lower bound; verify `c8@9` runs there and record
      whether its figures differ from Node 18's
- [ ] 2.3 Record the lowest figure across both runs for each of statements, branches, functions and
...
- [ ] 3.1 Replace `cli-app/.nycrc.json` with `c8` configuration preserving the existing `include`
...
- [ ] 3.2 Set `check-coverage` thresholds to the group 2 figures rounded **down** to whole numbers;
      verify `yarn coverage` exits `0` — the first time it has since `cli-app` became ESM
- [ ] 3.3 Add a comment in the config recording that the thresholds are a measured baseline to be
...
- [ ] 3.4 Prove the gate actually gates: delete or skip the `User identity` tests in
      `cli-app/test/key-manager.test.js`, run coverage, and verify it exits non-zero; restore them
      and verify it exits `0` again
- [ ] 3.5 Confirm `cli-app/coverage-cli-app/` and any `.nyc_output`/`.c8_output` directory are
      gitignored; verify `git status --short` is clean after a coverage run

## 4. Wire it into CI

- [ ] 4.1 Add a step running the mocha suite to `.github/workflows/cli-app-test-build.yaml`, after
      the existing `Yarn Install` step; verify it runs on both matrix entries and that a pushed
      branch shows the tests executing
- [ ] 4.2 Add a step running the coverage gate; verify a green run and, on a scratch branch with a
      test removed, a red one — the gate is not trusted until it has been seen to fail in CI
- [ ] 4.3 Decide from `design.md — Open Questions` whether coverage runs on the whole matrix or one
      Node version, and apply it; verify the workflow file reflects the decision explicitly rather
      than by omission

## 5. Correct the documentation

- [ ] 5.1 In the root `CLAUDE.md`, replace the claim that `.nycrc.json` enforces 70% with the real
      configuration and baseline; verify no occurrence of `70%` remains that refers to coverage
- [ ] 5.2 In the same file's CI section, remove the statement that the mocha suite is not wired into
      CI, and describe what CI now runs; verify it matches `.github/workflows/` as it stands after
      group 4
- [ ] 5.3 Correct the `.nvmrc` reference if needed — `CLAUDE.md` says "Node pinned to 18 by
      `.nvmrc`" while the file is at `cli-app/.nvmrc`; verify the path given resolves
- [ ] 5.4 Note in `CLAUDE.md` that `c8` is pinned because of the Node target, so the next person to
      modernise the runtime knows to unpin it; verify the note points at `design.md` for the detail

## 6. Close out

- [ ] 6.1 Run `cd cli-app && yarn test && yarn coverage`; verify both exit `0` and the reported
      figures match the thresholds set in group 3
- [ ] 6.2 Run `openspec validate fix-esm-coverage-reporting --type change --strict`; verify it
      passes with `specs` reported as skipped rather than missing
- [ ] 6.3 Confirm this change's amendments to `document-cli-contracts` still read correctly — its
