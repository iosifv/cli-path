...
- **THEN** the user is told the command is unknown and shown what is available
...
All user-facing output SHALL be produced through the CLI's shared presentation layer, so that
headings, values, status lines, tables and errors are styled consistently wherever they originate.

#### Scenario: A command reports a value

- **WHEN** any command displays a labelled value, a status line, a table or an error
